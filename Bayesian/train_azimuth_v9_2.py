#!/usr/bin/env python3
"""train_azimuth_v9_2.py — V9.2 SURGICAL TWEAKS"""
...
from V9_2_Bayesian_Model import (
    MultiTaskScalogramV9_2_Bayesian,
    build_v9_2_from_v8,
...
class Logger:
    def __init__(self, log_path):
        ...
        self._print_and_log(f"  V9.2 SURGICAL TWEAKS — TRAINING LOG")
...
def train_one_epoch(model, loader, optimizer, criterion, device,
    ...
    for batch in loader:
        ...
        # [V9.2 TASK 3] HEAVY GATE PENALTY: punish gate -> 0
...
def train(args):
    ...
    log = Logger(out_dir / "v9_2_training.log")
    ...
    log(f"Backbone LR     : {args.lr_backbone}")
    ...
    for batch in train_loader:
        ...
        if prior.shape[0] > 1:
            ...
            if diff > 0.001:
                log(f"[OK] DYNAMIC PRIORS VERIFIED: diff={diff:.4f}")
            else:
                log(f"[WARN] First two samples have similar priors")
        break

    log(f"\n{'-'*70}")
    log(f"  STEP 2: Building V9.2 Surgical Model")
    log(f"{'-'*70}")
    model = build_v9_2_from_v8(args.ckpt_v8, prior_dim=360, device=DEVICE)
    ...
    log(f"[INFO] Trainable parameter groups:")
    for name, param in model.named_parameters():
        if param.requires_grad:
            log(f"  {name}  ({param.numel():,} params)")

    log(f"\n{'-'*70}")
    log(f"  STEP 3: Optimizer & Scheduler (Differential LR)")
    log(f"{'-'*70}")
    criterion = VonMisesDirectionalLoss()

    # [V9.2 TASK 3] Differential parameter groups
    backbone_params = []
    head_params = []
    for name, param in model.named_parameters():
    ...
    log(f"[OK] Optimizer : AdamW (backbone_lr={args.lr_backbone}, head_lr={args.lr_head}, wd={args.weight_decay})")
    log(f"[OK] Scheduler : CosineAnnealingWarmRestarts (T_0=5, T_mult=1)")

    log(f"\n{'='*70}")
    log(f"  STARTING V9.2 TRAINING — SURGICAL TWEAKS")
    log(f"  Target: Val MAE < 25 deg")
    log(f"{'='*70}\n")
    best_val_mae = float('inf')
    ckpt_path = out_dir / "v9_2_surgical_best.pth"
    ...
    log(f"\n{'='*70}")
    log(f"  V9.2 TRAINING COMPLETE — SURGICAL TWEAKS")
    log(f"{'='*70}")
    ...
    log(f"{'='*70}")
    log.save_history_csv(out_dir / "v9_2_training_history.csv")
    log.close()


def parse_args():
    p = argparse.ArgumentParser(description="V9.2: Train Gated Bayesian Azimuth with Surgical Tweaks")
    ...
    p.add_argument("--lr-backbone", type=float, default=1e-5, help="Learning rate for surgically unfrozen backbone (very low)")
    p.add_argument("--lr-head", type=float, default=2e-4, help="Learning rate for azimuth head")
    p.add_argument("--weight-decay", type=float, default=0.05)
    p.add_argument("--gate-lambda", type=float, default=0.5, help="Heavy gate penalty coefficient (raised from 0.05)")
