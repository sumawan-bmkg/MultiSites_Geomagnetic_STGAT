...
# ============================================================================
# ARCHITECTURE (Must match train_stgat_v2.py)
# ============================================================================
...
def evaluate_v2():
    ...
    print("\n--- TAHAP 1: G5 Solar Storm Stress Test (V2 Manual Gating) ---")
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Testing G5 Storm"):
            ...
            if '2024-05-10' <= d_str <= '2024-05-12':
                ...
                raw_event, _, _, _, gate_val = model(x_img, x_cosmic)
                
                # V2 Manual Gating: prob = sigmoid(raw) * mean(gate)
                prob_raw = torch.sigmoid(raw_event).item()
                gate_scalar = gate_val.mean().item()
                prob_final = prob_raw * gate_scalar
                
                g5_probs.append(prob_final)
                g5_dates.append(d_str)

    g5_fpr = np.mean(np.array(g5_probs) >= 0.5) if g5_probs else 0
    print(f"G5 Storm Period Samples: {len(g5_probs)}")
    print(f"G5 False Positive Rate (FPR): {g5_fpr:.2%}")

    # 2. 2026 BLIND TEST EVALUATION
    print("\n--- TAHAP 2: 2026 Blind Test Evaluation (V2) ---")
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Running Blind Test"):
            x_img, x_cosmic, y_event, y_mag, y_azm, y_dist, date = batch
            x_img, x_cosmic = x_img.to(device), x_cosmic.to(device)
            raw_event, out_mag, out_azm, _, gate_val = model(x_img, x_cosmic)
            ...
            pred_event = 1 if prob_final >= 0.5 else 0
    ...
    # Save CSV
    df.to_csv('doc/2026_blindtest_v2_full.csv', index=False)
    print("Full results saved to doc/2026_blindtest_v2_full.csv")

    # Plots
    plt.figure(figsize=(12, 6))
    plt.subplot(1, 2, 1)
    plt.hist(df[df['y_event']==0]['prob_final'], bins=20, alpha=0.5, label='Noise')
    plt.hist(df[df['y_event']==1]['prob_final'], bins=20, alpha=0.5, label='Events')
    plt.axvline(0.5, color='r', linestyle='--')
    plt.title('V2: Probability Distribution (Gated)')
    plt.legend()
    ...
    if not df_tp.empty:
        ...
        plt.title('V2: Magnitude Regression')
    
    plt.tight_layout()
    plt.savefig('plots/final_evaluation_v2.png')
    print("Plot saved to plots/final_evaluation_v2.png")
